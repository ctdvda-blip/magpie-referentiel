...
dict(id=u"IA-01", titre=u"Spécifier un composant plutôt que le décrire",
     ...
     limite=u"La réponse valide la SORTIE du composant produit, pas son "
            u"code. Un script juste par accident — qui compterait autre "
            u"chose et tomberait sur dix — passerait. C'est pourquoi "
            u"l'énoncé interdit de corriger le code à la main : ce qui "
            u"s'apprend est la spécification, et elle se juge à la lecture.",
...
dict(id=u"IA-03", titre=u"Reprendre une demande sur le point qui échoue",
     ...
     donnees_note=u"28 niveaux répartis de part et d'autre du zéro, de −23 à "
                  ...
                  u"d'objet.",
     limite=u"L'amplitude juste prouve que la reprise a abouti, pas "
            u"qu'elle était BIEN formulée. Deux apprenants peuvent obtenir "
            u"48 en deux échanges ou en douze — l'écart est tout le sujet "
            u"de l'exercice, et il ne se mesure pas automatiquement.",
...
dict(id=u"IA-04", titre=u"Un composant scripté qui somme un métré",
     ...
     limite=u"58,03 m² est une surface DÉVELOPPÉE de tronçons droits. Les "
            u"coudes, les brides et les recouvrements de calorifuge s'y "
            u"ajoutent en atelier : le métré sert à commander, pas à "
            u"facturer.",
...
dict(id=u"IA-05", titre=u"Le code qui tourne et se trompe",
     ...
     limite=u"Neuf tronçons prouve que le défaut est CORRIGÉ, pas qu'il "
            u"est compris. Un apprenant peut remplacer le code entier par "
            u"le sien et obtenir la bonne valeur sans avoir jamais vu "
            u"l'erreur — l'exercice demande de la nommer, et cela se lit.",
     mode=u"SingleValue", tol=u"0", nb=4,
     comp=u"C# Script ou Python 3 Script, Panel",
...
dict(id=u"IA-06", titre=u"Transposer sans changer le résultat",
     ...
     limite=u"L'égalité des seize valeurs établit que les deux versions "
            u"s'accordent SUR CE JEU. Elle ne prouve pas l'équivalence des "
            u"deux codes : un cas limite absent du jeu — liste vide, valeur "
            u"négative — peut les séparer, et c'est le défaut classique du "
            u"portage.",
...
dict(id=u"IA-09", titre=u"Prédire une déperdition sur une baie nouvelle",
     ...
     limite=u"380 W est une estimation par RÉGRESSION sur vingt-quatre "
            u"points. Elle vaut dans la plage relevée ; extrapoler à une "
            u"baie de 8 m² sortirait du domaine des données, et le modèle "
            u"n'en dit rien. La tolérance de 10 W couvre le choix de la "
            u"méthode d'ajustement, pas la validité du modèle.",
...
dict(id=u"IA-10", titre=u"Regrouper un débit pour rationaliser la commande",
     ...
     limite=u"L'effectif du groupe le plus fourni valide le REGROUPEMENT, "
            u"pas son intérêt économique. Trois longueurs de commande "
            u"peuvent coûter plus cher que quatre selon le barème du "
            u"fournisseur et la chute engendrée — l'arbitrage est un calcul "
            u"de coût, pas de comptage.",
...
dict(id=u"IA-14", titre=u"Le résultat plausible et faux",
     ...
     limite=u"Le volume exact tranche la question posée. Il ne dit pas "
            u"COMMENT l'écart a été trouvé : un facteur mille se repère à "
            u"l'ordre de grandeur, et c'est ce réflexe — comparer au "
            u"plausible avant de comparer au juste — que l'exercice "
            u"travaille et que le nombre seul ne mesure pas.",
     mode=u"NumericTolerance", tol=u"0,01", nb=5,
