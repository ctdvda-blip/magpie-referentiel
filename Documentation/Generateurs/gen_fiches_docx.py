...
def bloc_sujet(doc, e, dossier, seul):
    ...
    # Le sujet est identique sur les deux fiches ; la valeur reste au corrige.
    para(doc, pour_apprenant(e))
...
def bloc_corrige(doc, e, dossier):
    ...
    if fuite(e):
        sous_titre(doc, u"La valeur attendue")
        encadre(doc, e["att"], fond="EAF1F8", taille=11)
        para(doc, u"Cette valeur ne figure pas sur la fiche remise à "
                  u"l'apprenant : elle y écrirait la réponse.",
...
def bloc_charniere(doc, e, dossier):
    ...
    for ligne in charniere_equilibree(e).split(chr(10)):
        if ligne.strip():
...
#: si non vide, seuls ces identifiants sont reecrits. Une fiche corrigee
#: apres coup ne doit pas rendre les 197 autres perimees : le convertisseur
#: PDF se fie aux dates, et reecrire tout lui redonnerait une heure de
#: travail pour rien. Se regle depuis l'appelant :
#:     import gen_fiches_docx as G ; G.SEULEMENT = {"AV-02"} ; G.main()
SEULEMENT = set()
...
def main():
    ...
    def _sauver(doc, chemin):
        try:
            ...
            return False
    for e in LOT_A:
        if SEULEMENT and e["id"] not in SEULEMENT:
            continue
