...
SORTIE = os.path.join(PROJET, "Fondamentaux Grasshopper - IndC - 01-09-2026.xlsx")
...
def main():
    ...
    # ------------------------- notions ajoutees pour l'equilibrage des
    #                           categories (indice C)
    try:
        import notions_complement
        ajout = notions_complement.lignes_referentiel(len(lignes) + 1)
        lignes.extend(ajout)
        print(u"Notions de complément : %d (REF-%03d à REF-%03d)"
              % (len(ajout), len(lignes) - len(ajout) + 1, len(lignes)))
    except Exception as _ex:
        print(u"ATTENTION : notions de complément non ajoutées (%s)" % _ex)

    # ------------------------------------------------ remise en ordre
    # Les notions ajoutees prennent les identifiants suivants — REF-143 et
    # au-dela — parce qu'un identifiant doit rester stable : toutes les fiches
    # d'exercice deja ecrites designent REF-065, REF-104 et les autres.
    #
    # Mais elles doivent se LIRE a leur place, avec les notions de leur
    # categorie. On trie donc les lignes par domaine puis par categorie, en
    # respectant l'ordre d'apparition de chacun — un tri alphabetique
    # casserait la progression pedagogique que porte l'ordre d'origine. Le tri
    # est STABLE : a domaine et categorie egaux, l'ordre initial est conserve.
    #
    # La colonne N° porte alors la position, l'ID porte l'identite. Les deux
    # cessent de coincider, et c'est le role de chacune.
    rang_dom, rang_cat = {}, {}
    for l in lignes:
        rang_dom.setdefault(l[2], len(rang_dom))
        rang_cat.setdefault((l[2], l[3]), len(rang_cat))
    lignes.sort(key=lambda l: (rang_dom[l[2]], rang_cat[(l[2], l[3])]))
