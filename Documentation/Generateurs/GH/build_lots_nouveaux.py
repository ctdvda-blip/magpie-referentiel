# -*- coding: utf-8 -*-
"""
MAGPIE - Production des lots RH, GP, QT, FA, MP, AV, PL, DV et WB.
=============================================================
...
# Ces exercices se repartissent sur neuf dossiers : le registre dit lequel.
SORTIE = os.path.join(PROJET, "EXERCICES")
...
from lots import TOUS as _TOUS, dossier_de
import recipes_nouveaux as _RN
LOT_A = [e for e in _TOUS if e["id"] in _RN.R]
RECETTES = dict(_RN.R)
...
def assembler():
    ...
    for f in LOT_A:
        ...
        if r is None:
            ...
            continue   # IA-07 : le livrable est un plugin compile, pas un .gh
        ex = dict(r)
        ex["id"] = f["id"]
        ex["titre"] = f["titre"]
        ex["niv"] = f["niv"]
...
# ------------------------------------------------------------------ production
def _dossier_exercice(eid):
    """Le dossier de l'exercice — CELUI QUI EXISTE s'il existe deja.

    Les exercices du lot A vivent dans des dossiers a titre long, herites de
    `build_lot_a.py` ; tous les autres portent le seul identifiant. Ecrire
    systematiquement dans le dossier court a duplique les quarante-neuf
    exercices du lot A : les definitions neuves d'un cote, leurs fiches de
    l'autre. On rejoint donc le dossier existant, quel que soit son nom, et
    on n'en cree un que s'il n'y en a aucun.
    """
    racine = os.path.join(PROJET, dossier_de(eid).replace("/", os.sep))
        ...
        for d in sorted(os.listdir(racine)):
            if d == eid or d.startswith(eid + " "):
                ...
                if os.path.isdir(chemin):
                    return chemin
    return os.path.join(racine, E._u(eid))


def main(dry=False):
    exos, manquantes = assembler()
    print("=" * 74)
    print("MAGPIE - production des nouveaux lots")
    print("=" * 74)
    ...
    ok, ko, avec_manuel = [], [], []
    for ex in exos:
        # Chaque exercice va dans le dossier de SON lot, nomme par son
        # seul identifiant : les titres complets faisaient deja depasser
        # la limite de chemin de Windows a la publication.
        dossier = _dossier_exercice(ex["id"])
        try:
    ...
    if ko:
        ...
        for i, msg in ko:
            print("   %-8s %s" % (i, msg))
    print("Lots produits : %s" % ", ".join(sorted(set(
        f["id"].split("-")[0] for f in LOT_A))))
    print("=" * 74)
